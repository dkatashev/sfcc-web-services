...
describe('scripts/webservice/WebDavService', () => {
  ...
  describe('#createRequest()', () => {
    it('should create a WebDAV request and set headers, path patterns, and client', () => {
      ...
      expect(request).to.equal(params);
      expect(svc.URL).to.equal('https://test.com/value');
    ...
    it('should call onCreateRequest callback if provided', () => {
      const onCreateRequestStub = sinon.stub();
      const params = {
        pathPatterns: { testParam: 'value' },
        headers: { 'Content-Type': 'application/xml' },
        onCreateRequest: onCreateRequestStub,
        getRequest: sinon.stub().returns('requestObject')
      };
      const svc = TestService._createService('default', params);
      ...
      expect(onCreateRequestStub.calledOnceWith(params, svc, svc.configuration.credential)).to.be.true;
    });
  });
